      ...
      <div className="container mx-auto max-w-8xl pt-14 px-4 sm:px-6 lg:px-0">
          ...
          <div className="grid grid-cols-12 sm:gap-10 gap-y-6">
            ...
            <div className="md:col-span-3 sm:col-span-6 col-span-12">
              <div className="flex flex-col gap-4 w-fit">
                {getFooterLinks().slice(0, 4).map((item, index) => (
                  <div key={index}>
                    <Link href={`/${locale}${item.href}`} className="text-white/40 text-xm hover:text-white">
                      {t(`links.${item.key}`)}
                    </Link>
                  </div>
                ))}
            ...
            <div className="md:col-span-2 sm:col-span-6 col-span-12">
              <div className="flex flex-col gap-4 w-fit">
                {getFooterLinks().slice(4, 8).map((item, index) => (
                  <div key={index}>
                    <Link href={`/${locale}${item.href}`} className="text-white/40 text-xm hover:text-white">
                      {t(`links.${item.key}`)}
                    </Link>
        ...
        <div className="flex justify-between md:flex-nowrap flex-wrap items-center py-6 gap-6">
          <p className="text-white/40 text-sm ">
            {t('copyright')} <Link href="https://getnextjstemplates.com/" className="hover:text-primary" target="_blanck">GetNextJs Templates</Link>
          </p>
