...
export type GeoListingRouteParams = { locale: string; country: string; city: string; filters?: string[] };

/**
 * One listing, two routes.
 *
 * `[locale]/[country]/[city]/[[...filters]]` serves every URL without a query
 * string and never sees `searchParams` — awaiting that prop is what makes a
 * route render per request, and it is why every listing URL answered
 * `private, no-store` until 2026-09-20. URLs that do carry a query are
 * rewritten by the middleware to `[locale]/listing-query/…`, which reads the
 * query and calls the same two functions below with it.
 */
type Props = {
  params: GeoListingRouteParams;
  /** The URL's query; `{}` on the cached, path-only route. */
  search: SearchParams;
};
...
export async function geoListingMetadata({ params, search }: Props): Promise<Metadata> {
  const { locale, country, city, filters = [] } = params;
...
export async function GeoListingPage({ params, search }: Props) {
  const { locale, country, city, filters = [] } = params;
