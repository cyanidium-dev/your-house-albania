...
import { FilterMultiSelect, type FilterMultiOption } from '@/components/catalog/FilterMultiSelect';
...
export function PropertySearchBar({
  ...
  }, [amenities, beds, city, deal, maxPrice, minPrice, pageSize, pathname, router, searchParams, sort, type, district]);
  ...
  const locationOptions: FilterOption[] = locations.map((o) => ({ value: o.value, label: o.label }));
  const propertyTypeOptions: FilterOption[] = propertyTypes.map((o) => ({ value: o.value, label: o.label }));
  const dealTypeOptions: FilterOption[] = dealTypes.map((o) => ({ value: o.value, label: o.label }));
  const amenityMultiOptions: FilterMultiOption[] = amenityOptions.map((o) => ({ value: o.value, label: o.label }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            ...
            <Slider.Thumb className="block size-4 rounded-full border border-white bg-primary shadow focus:outline-none focus:ring-2 focus:ring-primary/40" />
            <Slider.Thumb className="block size-4 rounded-full border border-white bg-primary shadow focus:outline-none focus:ring-2 focus:ring-primary/40" />
          </Slider.Root>
        </div>
        ...
        <div className="flex items-end gap-2 justify-end">
          ...
          <Button type="submit" className="h-10 px-6 rounded-full cursor-pointer">
            Search
          </Button>
        </div>
      </div>
      ...
      {/* ADVANCED FILTERS */}
      {showAdvanced && (
        <div className="pt-4 border-t border-dark/5 dark:border-white/10">
          <div className="grid gap-4 grid-cols-1 md:grid-cols-3 lg:grid-cols-5">
            ...
            <FilterMultiSelect
              ...
              summaryLabel={(count) => (count === 0 ? 'Amenities' : `${count} selected`)}
            />
