...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const title = catalogSeo?.metaTitle || `${t("title")} — rent`;
  ...
  const robots =
    shouldCatalogListingNoindex({ ...search, deal: "rent" }, { ignoredQueryKeys: ["deal"] }) || (catalogSeo?.noIndex ?? false)
...
export default async function RentFilterPage({ params, searchParams }: Props) {
  const [{ locale }, search] = await Promise.all([params, searchParams]);
  const mergedSearch = { ...search, deal: "rent" };
  const t = await getTranslations("Listing.properties");
